...
using Coal.Classes;
using HarmonyLib;
using Coal.Notifications;
using System;
using System.Linq;
using UnityEngine;
using UnityEngine.InputSystem;
using UnityEngine.UI;
using static Coal.Menu.Buttons;
...
namespace Coal.Menu
{
    ...
    public class Main : MonoBehaviour
    {
            ...
            // Buttons
                // Disconnect
                    if (disconnectButton)
                    {
                        ...
                        disconnectbutton.AddComponent<Coal.Classes.Button>().relatedText = "Disconnect";
                ...
                // Page Buttons
                    ...
                    gameObject.AddComponent<Coal.Classes.Button>().relatedText = "PreviousPage";
                    ...
                    gameObject.transform.localPosition = new Vector3(0.56f, -0.65f, 0);
                    gameObject.GetComponent<Renderer>().material.color = buttonColors[0].colors[0].color;
                    gameObject.AddComponent<Coal.Classes.Button>().relatedText = "NextPage";
        ...
        public static void CreateButton(float offset, ButtonInfo method)
        {
            ...
            gameObject.AddComponent<Coal.Classes.Button>().relatedText = method.buttonText;
                ...
                if (TPC != null)
                {
                    ...
                    if (reference != null)
                    {
                        if (Mouse.current.leftButton.isPressed)
                        {
                            ...
                            if (worked)
                            {
                                Coal.Classes.Button collide = hit.transform.gameObject.GetComponent<Coal.Classes.Button>();
                                if (collide != null)
                                {
        ...
        public static void Cleanup()
        {
            try
            {
                ...
                {
                    Transform vcamTransform = shoulderCamera.transform.Find("CM vcam1");
                    if (vcamTransform != null)
                    {
                        vcamTransform.gameObject.SetActive(true);
                    }
                }
                ...
                if (menu != null)
                {
                    ...
                    if (fpsObject != null)
                    {
                        fpsObject.text = "FPS: N/A";
                    }

                    UnityEngine.Debug.Log($"{PluginInfo.Name} // Cleanup completed successfully.");
                }
            }
            catch (Exception exc)
            {
                UnityEngine.Debug.LogError($"{PluginInfo.Name} // Error during cleanup: {exc.StackTrace} - {exc.Message}");
            }
        }
